...
export function NotificationBell() {
  ...
  return (
    <div ref={bellRef} className="relative z-10">
      {/* ── Botón campana ── */}
      <button
        onClick={() => setOpen((v) => !v)}
      ...
      {/* ── Panel desplegable ── */}
      {open && (
        <div className="fixed top-[4.5rem] bg-white rounded-2xl shadow-xl border border-zinc-200 z-40 overflow-hidden w-[calc(100vw-2rem)] max-w-sm left-1/2 -translate-x-1/2 sm:w-80 sm:max-w-none sm:left-auto sm:right-6 sm:translate-x-0">
            ...
            <div className="flex items-center gap-1">
              ...
              {unreadCount > 0 && (
                <button
                  ...
                  disabled={markingAll}
                  className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs text-zinc-500 hover:text-primary hover:bg-zinc-100 transition-colors disabled:opacity-50"
                ...
                {notifications.map((notif) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm leading-snug ${notif.isRead ? 'text-zinc-600' : 'text-zinc-900 font-medium'}`}>
                        {notif.message}
                      </p>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="font-mono text-xs text-zinc-400">{notif.saleCode}</span>
                        <span className="text-[10px] text-zinc-400">·</span>
                        <span className="text-xs text-zinc-400">{timeAgo(notif.createdAt)}</span>
                      </div>
                    ...
                    {notif.isRead && (
                      <Check className="w-3.5 h-3.5 text-zinc-300 mt-1 flex-shrink-0" />
                    )}
                  </button>
                ))}
              </div>

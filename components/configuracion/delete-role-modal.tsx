        ...
        <div className="bg-zinc-50 rounded-lg p-4 border border-zinc-200">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-zinc-900 font-semibold">{role.name}</span>
            </div>
            {role.description && (
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-zinc-700">Descripción:</span>
                <span className="text-sm text-zinc-900">{role.description}</span>
              </div>
            )}
            {role.usersCount !== undefined && role.usersCount > 0 && (
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-zinc-700">Usuarios asignados:</span>
                <span className="text-sm text-zinc-900 font-semibold">{role.usersCount}</span>
              </div>
            )}
          </div>
        </div>

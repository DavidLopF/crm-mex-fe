...
// ── Parser: Formato El Carretel ───────────────────────────────────────────────
// Columnas (fila 4 como encabezado):
//   0: GrupoUno  1: GrupoDos  2: CódigoInventario  3: Descripción
//   4: U Medida  5: Existencias  6: Precio 1  7: Precio 2  8: Precio 3  9: Precio 4
function parseExcelCarretel(wb: XLSX.WorkBook): ParsedImportRow[] {
  const ws = wb.Sheets[wb.SheetNames[0]];
  // Leer todas las filas como array de arrays (incluye filas vacías)
  const allRows: unknown[][] = XLSX.utils.sheet_to_json(ws, { header: 1, defval: null }) as unknown[][];

  const results: ParsedImportRow[] = [];

  // Carry-forward: GrupoUno y GrupoDos solo aparecen en la primera fila de cada grupo.
  // Las filas siguientes del mismo grupo tienen estas celdas vacías, por lo que
  // debemos recordar el último valor no vacío visto.
  let lastGrupoUno = '';
  let lastGrupoDos = '';
  ...
  for (let i = 4; i < allRows.length; i++) {
    ...
    // Actualizar carry-forward de grupos cuando la celda no está vacía
    const grupoUnoRaw = String(row[0] ?? '').trim();
    const grupoDosRaw = String(row[1] ?? '').trim();
    if (grupoUnoRaw && !grupoUnoRaw.toLowerCase().startsWith('total')) {
      lastGrupoUno = grupoUnoRaw;
    }
    if (grupoDosRaw && !grupoDosRaw.toLowerCase().startsWith('total')) {
      lastGrupoDos = grupoDosRaw;
    }

    const codigoRaw = row[2];
    const descripcionRaw = row[3];

    // Saltar filas de totales/subtotales y filas sin código de inventario
    ...
    const uMedida   = String(row[4] ?? '').trim();
    const existencias = row[5];
    const precio1   = row[6];

    // Usar la subcategoría (GrupoDos) si existe, si no la categoría principal (GrupoUno)
    const categoryName = lastGrupoDos || lastGrupoUno || 'General';

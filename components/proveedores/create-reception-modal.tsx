      ...
      <Modal isOpen={isOpen} onClose={handleClose} title="Recepción Registrada" size="xl">
        <div className="space-y-5">
          ...
          <div className="flex flex-col items-center py-4">
            ...
            <h3 className="text-lg font-bold text-zinc-900">Recepción creada exitosamente</h3>
            <p className="text-sm text-zinc-500 font-mono font-semibold text-green-700">
              {receipt.code}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-zinc-50 rounded-lg p-3 border border-zinc-200">
              <p className="text-[10px] text-zinc-400 mb-0.5">Almacén</p>
              <div className="flex items-center gap-1.5">
                <Warehouse className="w-3.5 h-3.5 text-zinc-500" />
                <span className="text-sm font-medium text-zinc-900">{warehouseName}</span>
              </div>
            </div>
            <div className="bg-zinc-50 rounded-lg p-3 border border-zinc-200">
              <p className="text-[10px] text-zinc-400 mb-0.5">Fecha</p>
          ...
          <div className="bg-zinc-50 rounded-lg border border-zinc-200 overflow-hidden">
            <div className="px-4 py-2.5 border-b border-zinc-200 bg-zinc-100/50 flex items-center justify-between">
              <span className="text-xs font-semibold text-zinc-600">Artículos Recibidos</span>
          ...
          {progress && (
            <div className="bg-zinc-50 rounded-lg p-3 border border-zinc-200">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-semibold text-zinc-600">Progreso de la Orden</span>
    ...
    <Modal isOpen={isOpen} onClose={handleClose} title="Registrar Recepción" size="xl">
      <div className="space-y-5 max-h-[75vh] overflow-y-auto pr-1">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            {loadingWarehouses ? (
              <div className="flex items-center gap-2 py-2">
                ...
                <span className="text-xs text-zinc-400">Cargando almacenes...</span>
              </div>
            ) : warehouses.length > 0 ? (
              <select
                value={warehouseId}
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-700 mb-1">Fecha de Recepción</label>
            <input
              type="date"
              value={receivedDate}
        ...
        <div>
          <label className="block text-xs font-medium text-zinc-700 mb-1">Notas Generales</label>
          <textarea
            value={generalNotes}
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-sm font-semibold text-zinc-700">Artículos Pendientes</h4>
            <button
              onClick={selectAll}
              ...
              {lines.map((line, idx) => (
                <div
                  ...
                  className={`bg-white rounded-lg p-3 border transition-colors ${
                    line.selected ? 'border-blue-300 bg-blue-50/30' : 'border-zinc-200 opacity-60'
                  }`}
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <label className="text-[10px] text-zinc-500">Recibir:</label>
                      <input
                        type="number"
                        min={0}
                        max={line.qtyPending}
                        ...
                        className="w-16 border border-zinc-200 rounded px-2 py-1 text-sm text-center focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-zinc-100 disabled:cursor-not-allowed"
                      />
                    </div>
                  </div>
                  {line.selected && (
                    <div className="mt-2 ml-7">
                      <input
                        ...
                        className="w-full border border-zinc-100 rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400"
                      />
                    </div>
                  )}
                </div>
        ...
        {selectedLines.length > 0 && (
          <div className="bg-zinc-50 rounded-lg p-3 border border-zinc-200">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-bold text-zinc-900">{totalUnits} unidades a recibir</span>
            </div>
          </div>
        )}
